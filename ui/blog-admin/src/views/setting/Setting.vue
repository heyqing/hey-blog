    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="修改信息" name="info">
        <div class="info-container">
          <el-upload class="avatar-uploader" :action='`${baseUrl}/users/avatar`' :show-file-list="false"
            :headers="headers" :on-success="updateAvatar">
      ...
      <el-tab-pane label="修改密码" name="password">
        <el-form label-width="70px" :model="passwordForm" style="width: 320px">
          <el-form-item label="旧密码">
            <el-input @keyup.enter.native="updatePassword" v-model="passwordForm.oldPassword" size="small"
              show-password />
          </el-form-item>
          <el-form-item label="新密码">
            <el-input @keyup.enter.native="updatePassword" v-model="passwordForm.newPassword" size="small"
              show-password />
          </el-form-item>
          <el-form-item label="确认密码">
            <el-input @keyup.enter.native="updatePassword" v-model="passwordForm.confirmPassword" size="small"
              show-password />
          </el-form-item>
  ...
  methods: {
    handleClick(tab) {
      if (tab.index == 2 && this.notice == '') {
        this.axios.get(`${baseUrl}/admin/notice`).then(({ data }) => {
    ...
    updateInfo() {
      ...
      this.axios.put(`${baseUrl}/users/info`, this.infoForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '修改成功'
    ...
    updatePassword() {
      ...
      this.axios.put(`${baseUrl}/admin/users/password`, this.passwordForm).then(({ data }) => {
        if (data.flag) {
