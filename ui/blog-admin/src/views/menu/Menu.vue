  ...
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <div style="margin-left: auto">
        <el-input v-model="keywords" prefix-icon="el-icon-search" size="small" placeholder="请输入菜单名" style="width: 200px"
    ...
    <el-table v-loading="loading" :data="menus" row-key="id"
      ...
      <el-table-column prop="isHidden" label="隐藏" align="center" width="80">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.isHidden" active-color="#13ce66" inactive-color="#F4F4F5" :active-value="1"
...
export default {
  ...
  methods: {
    ...
    changeDisable(menu) {
      ...
      this.axios.put(`${baseUrl}/admin/menus/isHidden`, params).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '修改成功'
    ...
    saveOrUpdateMenu() {
      ...
      this.axios.post(`${baseUrl}/admin/menus`, this.menuForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '操作成功'
    ...
    deleteMenu(id) {
      this.axios.delete(`${baseUrl}/admin/menus/` + id).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '删除成功'
