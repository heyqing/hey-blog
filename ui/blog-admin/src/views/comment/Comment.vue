<template>
  <el-card class="main-card">
    ...
    <div class="operation-container">
      <el-button type="danger" size="small" icon="el-icon-delete" :disabled="commentIds.length == 0"
        @click="remove = true">
        批量删除
      </el-button>
      <el-button type="success" size="small" icon="el-icon-success" :disabled="commentIds.length == 0"
...
export default {
  ...
  methods: {
    ...
    updateCommentReview(id) {
      ...
      param.isReview = 1
      this.axios.put(`${baseUrl}/admin/comments/review`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    deleteComments(id) {
      ...
      this.axios.delete(`${baseUrl}/admin/comments`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
