  ...
  <el-card class="main-card">
    <el-form ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="任务名称" prop="jobName">
        <el-input v-model="searchParams.jobName" placeholder="请输入任务名称" clearable size="small"
          @keyup.enter.native="listJobs" />
      </el-form-item>
      <el-form-item label="任务组名" prop="jobGroup">
        <el-select @change="listJobs" v-model="searchParams.jobGroup" placeholder="请选择任务组名" clearable size="small">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="danger" size="small" icon="el-icon-delete" :disabled="jobIds.length == 0"
          @click="isDelete = true">
          批量删除
        </el-button>
      </el-col>
    ...
    <div style="margin-top: 5px">
      <el-table border style="width: 100%" :data="jobs" @selection-change="selectionChange" v-loading="loading">
        ...
        <el-table-column label="状态" align="center">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.status" active-color="#13ce66" inactive-color="#F4F4F5" :active-value="1"
...
export default {
  ...
  methods: {
    ...
    listJobGroups() {
      this.axios.get(`${baseUrl}/admin/jobs/jobGroups`).then(({ data }) => {
    ...
    deleteJobs(id) {
      ...
      this.axios.delete(`${baseUrl}/admin/jobs`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '删除成功'
    ...
    handleChange(jobId) {
      ...
      this.title = '编辑任务'
      this.axios.get(`${baseUrl}/admin/jobs/` + jobId).then(({ data }) => {
        this.job = data.data
      })
      this.dialogFormVisible = true
    },
    ...
    handleEditOrUpdate() {
      if (this.editOrUpdate === true) {
        this.axios.put(`${baseUrl}/admin/jobs`, this.job).then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: '修改成功',
        ...
        this.axios.post(`${baseUrl}/admin/jobs`, this.job).then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: '添加成功',
              message: data.message
    ...
    handleRun(job) {
      ...
      this.axios.put(`${baseUrl}/admin/jobs/run`, params).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '执行成功',
            message: data.message
