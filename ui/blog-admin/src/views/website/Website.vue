      ...
      <el-tab-pane label="网站信息" name="info">
        <el-form label-width="100px" :model="websiteConfigForm" label-position="left">
          <el-form-item label="作者头像">
            <el-upload class="avatar-uploader" :action='`${baseUrl}/admin/config/images`' :headers="headers"
...
export default {
  ...
  methods: {
    getWebsiteConfig() {
      this.axios.get(`${baseUrl}/admin/website/config`).then(({ data }) => {
    ...
    updateWebsiteConfig() {
      this.axios.put(`${baseUrl}/admin/website/config`, this.websiteConfigForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
