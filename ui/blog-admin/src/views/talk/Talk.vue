    ...
    <div class="talk-container">
      ...
      <div class="operation-wrapper">
        <div class="left-wrapper">
          <el-upload :action='`${baseUrl}/admin/talks/images`' multiple :headers="headers" :before-upload="beforeUpload"
      ...
      <el-upload class="talk-image-upload" v-show="uploads.length > 0" :action='`${baseUrl}/admin/talks/images`'
        list-type="picture-card" multiple :headers="headers" :file-list="uploads" :before-upload="beforeUpload"
        :on-success="upload" :on-remove="handleRemove">
...
export default {
  ...
  created() {
    if (this.$route.params.talkId) {
      this.axios.get(`${baseUrl}/admin/talks/` + this.$route.params.talkId).then(({ data }) => {
        this.talk = data.data
        if (data.data.imgs) {
  ...
  methods: {
    ...
    saveOrUpdateTalk() {
      ...
      this.axios.post(`${baseUrl}/admin/talks`, this.talk).then(({ data }) => {
        if (data.flag) {
