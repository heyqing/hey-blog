    ...
    <el-form :inline="true" ref="form" label-width="68px">
      <el-row>
        ...
        <el-form-item label="任务组名">
          <el-select size="small" @change="listJobLogs" clearable style="margin-left: 5px"
            v-model="searchParams.jobGroup" placeholder="请选择组名">
...
export default {
  ...
  methods: {
    ...
    listJobGroups() {
      this.axios.get(`${baseUrl}/admin/jobLogs/jobGroups`).then(({ data }) => {
    ...
    deleteJobLogs() {
      ...
      this.axios.delete(`${baseUrl}/admin/jobLogs`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '删除成功'
    ...
    clean() {
      this.axios.delete(`${baseUrl}/admin/jobLogs/clean`).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: '清空成功'
