  ...
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <div style="margin-left: auto">
        ...
        <el-input v-model="keywords" prefix-icon="el-icon-search" size="small" placeholder="请输入相册名" style="width: 200px"
    ...
    <el-dialog :visible.sync="addOrEdit" width="35%" top="10vh">
      ...
      <el-form label-width="80px" size="medium" :model="albumForum">
        ...
        <el-form-item label="相册封面">
          <el-upload class="upload-cover" drag :headers="headers" :before-upload="beforeUpload"
            :action='`${baseUrl}/admin/photos/albums/upload`' multiple :on-success="uploadCover">
...
export default {
  ...
  methods: {
    ...
    listAlbums() {
      this.axios
        .get(`${baseUrl}/admin/photos/albums`, {
          params: {
    ...
    addOrEditAlbum() {
      ...
      this.axios.post(`${baseUrl}/admin/photos/albums`, this.albumForum).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    deleteAlbum() {
      this.axios.delete(`${baseUrl}/admin/photos/albums/` + this.albumForum.id).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
