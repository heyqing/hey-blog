<template>
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <el-button type="danger" size="small" icon="el-icon-delete" :disabled="this.categoryIds.length == 0"
        @click="isDelete = true">
        批量删除
      </el-button>
      <div style="margin-left: auto">
        <el-input v-model="keywords" prefix-icon="el-icon-search" size="small" placeholder="请输入分类名" style="width: 200px"
...
export default {
  ...
  methods: {
    ...
    deleteCategory(id) {
      ...
      this.axios.delete(`${baseUrl}/admin/categories`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    addOrEditCategory() {
      ...
      this.axios.post(`${baseUrl}/admin/categories`, this.categoryForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
