<template>
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <el-button type="danger" size="small" icon="el-icon-delete" :disabled="tagIds.length == 0"
        @click="isDelete = true">
        批量删除
      </el-button>
...
export default {
  ...
  methods: {
    ...
    deleteTag(id) {
      ...
      this.axios.delete(`${baseUrl}/admin/tags`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    addOrEditTag() {
      ...
      this.axios.post(`${baseUrl}/admin/tags`, this.tagForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
