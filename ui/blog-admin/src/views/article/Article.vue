    ...
    <el-dialog :visible.sync="addOrEdit" width="40%" top="3vh">
      ...
      <el-form label-width="80px" size="medium" :model="article">
        ...
        <el-form-item label="上传封面">
          <el-upload class="upload-cover" drag :action='`${baseUrl}/admin/articles/images`' multiple :headers="headers"
...
export default {
  created() {
    ...
    if (articleId) {
      this.axios.get(`${baseUrl}/admin/articles/` + articleId).then(({ data }) => {
  ...
  methods: {
    ...
    uploadImg(pos, file) {
      ...
      if (file.size / 1024 < this.config.UPLOAD_SIZE) {
        ...
        this.axios.post(`${baseUrl}/admin/articles/images`, formdata).then(({ data }) => {
    ...
    saveArticleDraft() {
      ...
      this.axios.post(`${baseUrl}/admin/articles`, this.article).then(({ data }) => {
        if (data.flag) {
    ...
    searchCategories(keywords, cb) {
      this.axios
        .get(`${baseUrl}/admin/categories/search`, {
          params: {
            keywords: keywords
          }
        })
