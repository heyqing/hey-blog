  ...
  <el-card class="main-card">
    ...
    <div class="operation-container">
      <el-button v-if="isDelete == 0" type="danger" size="small" icon="el-icon-delete"
        :disabled="articleIds.length == 0" @click="updateIsDelete = true">
        批量删除
      </el-button>
      ...
      <el-upload :action='`${baseUrl}/admin/articles/import`' multiple :limit="9" :show-file-list="false"
        :headers="uploadHeaders" :on-success="uploadArticle">
...
export default {
  ...
  methods: {
    ...
    updateArticleDelete(id) {
      ...
      this.axios.put(`${baseUrl}/admin/articles`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    deleteArticles(id) {
      ...
      this.axios.delete(`${baseUrl}/admin/articles/delete`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
    ...
    exportArticles(id) {
      ...
      this.axios.post(`${baseUrl}/admin/articles/export`, param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
    ...
    changeTopAndFeatured(article) {
      this.axios
        .put(`${baseUrl}/admin/articles/topAndFeatured`, {
          id: article.id,
          isTop: article.isTop,
    ...
    listCategories() {
      this.axios.get(`${baseUrl}/admin/categories/search`).then(({ data }) => {
        this.categories = data.data
      })
    },
    listTags() {
      this.axios.get(`${baseUrl}/admin/tags/search`).then(({ data }) => {
        this.tags = data.data
      })
    }
  },
