  ...
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <div style="margin-left: auto">
        <el-input v-model="keywords" prefix-icon="el-icon-search" size="small" placeholder="请输入资源名" style="width: 200px"
    ...
    <el-table v-loading="loading" :data="resources" row-key="id"
      ...
      <el-table-column prop="isAnonymous" label="匿名访问" align="center">
        <template slot-scope="scope">
          <el-switch v-if="scope.row.url" v-model="scope.row.isAnonymous" active-color="#13ce66"
            inactive-color="#F4F4F5" :active-value="1" :inactive-value="0" @change="changeResource(scope.row)" />
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    changeResource(resource) {
      this.axios.post(`${baseUrl}/admin/resources`, resource).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    deleteResource(id) {
      this.axios.delete(`${baseUrl}/admin/resources/` + id).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
    ...
    addOrEditResource() {
      ...
      this.axios.post(`${baseUrl}/admin/resources`, this.resourceForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
            message: data.message
